...
export const AnalysisReport: React.FC<AnalysisReportProps> = ({ result }) => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            {result.keyStrengths && result.keyStrengths.length > 0 ? (
              result.keyStrengths.map((strength, idx) => (
                <li key={idx}>
                  ...
                  ({(strength.confidence * 100).toFixed(1)}% confidence)
                </li>
              ))
            ) : (
        ...
        <div className="bg-yellow-50 rounded-lg p-4">
          ...
          <ul className="list-disc list-inside text-yellow-700">
            {result.areasOfNote && result.areasOfNote.length > 0 ? (
              result.areasOfNote.map((area, idx) => (
                <li key={idx}>
                  <strong>{area.title}:</strong> {area.description}{' '}
                  {area.significance && `(${area.significance})`}
                </li>
              ))
            ) : (
              <li>No areas of note identified.</li>
            )}
          </ul>
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white rounded-lg p-4 border border-gray-200">
                ...
                <p className="text-gray-700">{result.questionResponse.responseStyle}</p>
              </div>
              <div className="bg-white rounded-lg p-4 border border-gray-200">
                <h4 className="font-medium text-gray-900 mb-2">Topic Handling</h4>
                <p className="text-gray-700">{result.questionResponse.topicHandling}</p>
              </div>
            </div>
            <div className="bg-white rounded-lg p-4 border border-gray-200">
              <h4 className="font-medium text-gray-900 mb-2">Behavioral Patterns</h4>
              <ul className="list-disc list-inside text-gray-700 space-y-2">
                {result.questionResponse.behavioralPatterns.map((pattern, idx) => (
      ...
      {/* New Section: Question Impacts */}
      {result.questionImpacts && result.questionImpacts.length > 0 && (
        <div className="mt-8 border-t pt-6">
          <h3 className="text-lg font-semibold mb-4">Questions Impacting Mood</h3>
          <ul className="list-disc list-inside text-gray-700 space-y-2">
            {result.questionImpacts.map((impact, idx) => (
              <li key={idx}>
                <strong>Question:</strong> {impact.question} <br />
                <strong>Timestamp:</strong> {impact.timestamp} <br />
                <strong>Mood Change:</strong> {impact.moodChange} <br />
                {impact.analysis && <><strong>Analysis:</strong> {impact.analysis} <br /></>}
                {impact.confidence && (
                  <span>
                    <strong>Confidence:</strong> {(impact.confidence * 100).toFixed(1)}%
                  </span>
                )}
              </li>
            ))}
